...
def get_input_file_and_output_dir(
    ...
    while True:
        ...
        if os.path.exists(file_path):
            ...
            output_dir = os.path.join(base_output_dir, "RandomForest", subdir)
...
def train_model(X_train, y_train):
    """
    构建并训练随机森林分类器。
    随机森林是 Bagging + 决策树的集成方法，通过多棵树投票降低方差。
    常用超参数说明：
      - n_estimators    : 森林中决策树数量
      - max_depth       : 每棵树最大深度，None 表示不限制
      - max_features    : 每次分裂随机选取的特征数量（'sqrt' = √n_features）
      - min_samples_leaf: 叶子节点最少样本数，防止过拟合
...
def evaluate_model(model, X_test, y_test, output_dir: str):
    ...
    ax_roc.plot(fpr, tpr, color='steelblue', lw=2,
                label=f'Random Forest ROC (AUC = {auc:.4f})')
    ...
    ax_roc.set_title('ROC 曲线 - 信用风险评估模型（随机森林）', fontsize=15, fontweight='bold')
...
# =============================================================================
# 6. 特征重要性（随机森林自带 Gini/MDI 重要性）
# =============================================================================
def plot_feature_importance(model, feature_names, output_dir: str):
    """
    使用随机森林内置的基尼重要性（Mean Decrease in Impurity, MDI）。
    每棵树分裂时，某特征使基尼系数平均降低量越大，该特征越重要。
    """
    ...
    ax_fi.barh(
        ...
        xerr=[std[i] for i in reversed(indices)],
        color='steelblue',
        ecolor='black',
        capsize=3,
    )
    ax_fi.set_xlabel('基尼重要性（Mean Decrease in Impurity）', fontsize=12)
    ax_fi.set_title('随机森林特征重要性（基尼不纯度下降）', fontsize=14, fontweight='bold')
...
def main():
    ...
    print("     ├── feature_importance.png 特征重要性（基尼不纯度下降）")
    if SHAP_AVAILABLE:
