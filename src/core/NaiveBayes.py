...
def get_input_file_and_output_dir(
    ...
    while True:
        ...
        if os.path.exists(file_path):
            ...
            output_dir = os.path.join(base_output_dir, "NaiveBayes", subdir)
...
def train_model(X_train, y_train):
    """
    构建并训练高斯朴素贝叶斯分类器。
    GaussianNB 假设每个特征服从高斯（正态）分布，适合连续型特征。
    常用超参数说明：
      - var_smoothing : 方差平滑参数，防止数值为 0 导致的计算问题
                        默认 1e-9，可适当调大以增强泛化能力
    注意：朴素贝叶斯假设特征间相互独立，银行数据中特征往往有相关性，
          因此准确性可能不如集成方法，但速度极快，可解释性强。
    """
    neg_count = (y_train == 0).sum()
    pos_count = (y_train == 1).sum()
    print(f"\n[模型训练] 训练集样本：好客户 {neg_count} 个，坏客户 {pos_count} 个")
    print(f"[模型训练] 朴素贝叶斯：使用先验概率自然处理类别不平衡")

    model = GaussianNB(var_smoothing=1e-9)
    model.fit(X_train, y_train)
    print("[模型训练] 朴素贝叶斯训练完成！")
    return model
...
def evaluate_model(model, X_test, y_test, output_dir: str):
    ...
    ax_roc.plot(fpr, tpr, color='steelblue', lw=2,
                label=f'Naive Bayes ROC (AUC = {auc:.4f})')
    ...
    ax_roc.set_title('ROC 曲线 - 信用风险评估模型（朴素贝叶斯）', fontsize=15, fontweight='bold')
...
# =============================================================================
# 6. 特征重要性（朴素贝叶斯：各特征的类条件均值差异）
# =============================================================================
def plot_feature_importance(model, feature_names, output_dir: str):
    """
    朴素贝叶斯无传统特征重要性。
    使用各特征在两类别下条件均值的差异绝对值作为"区分度"指标。
    差异越大，该特征对区分好/坏客户越有价值。
    """
    if len(model.theta_) == 2:
        diff = np.abs(model.theta_[1] - model.theta_[0])
    else:
        diff = np.max(model.theta_, axis=0) - np.min(model.theta_, axis=0)

    indices = np.argsort(diff)[::-1][:15]

    fig_fi, ax_fi = plt.subplots(figsize=(9, 6))
    ax_fi.barh(
        [feature_names[i] for i in reversed(indices)],
        [diff[i] for i in reversed(indices)],
        color='steelblue',
    )
    ax_fi.set_xlabel('类别条件均值差异（特征区分度）', fontsize=12)
    ax_fi.set_title('朴素贝叶斯特征重要性（类条件均值差异）', fontsize=14, fontweight='bold')
...
def plot_shap_summary(model, X_train, output_dir: str):
    ...
    print("\n[SHAP] 正在计算 SHAP 值（朴素贝叶斯使用 KernelExplainer，可能较慢）...")
...
def main():
    ...
    print("     ├── feature_importance.png 特征重要性（类条件均值差异）")
    if SHAP_AVAILABLE:
