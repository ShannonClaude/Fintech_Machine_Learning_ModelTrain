...
import pandas as pd
import matplotlib
matplotlib.use('Agg')  # 非交互式后端，避免在无 GUI 环境报错
...
def get_input_file_and_output_dir(
    ...
    while True:
        ...
        if os.path.exists(file_path):
            ...
            output_dir = os.path.join(base_output_dir, "XGBoost", subdir)
...
def train_model(X_train, y_train):
    ...
    常用超参数说明：
      ...
      - learning_rate   : 学习率（eta），越小越保守
      - subsample       : 每轮使用样本比例，防止过拟合
      - colsample_bytree: 每棵树使用特征比例，防止过拟合
...
def evaluate_model(model, X_test, y_test, output_dir: str):
    ...
    ax_roc.plot(fpr, tpr, color='steelblue', lw=2,
                label=f'XGBoost ROC (AUC = {auc:.4f})')
    ...
    ax_roc.set_title('ROC 曲线 - 信用风险评估模型（XGBoost）', fontsize=15, fontweight='bold')
...
def plot_feature_importance(model, output_dir: str):
    """
    使用 XGBoost 内置的 plot_importance 绘制特征重要性图。
    importance_type 可选：'weight'（分裂次数）/ 'gain'（平均增益）/ 'cover'
    """
