...
# 核心模型：逻辑回归 (Logistic Regression) 分类器
# =============================================================================
...
#    本脚本位于 src/core/，因此：
#      输入目录 → ../../data/input
#      输出目录 → ../../output/LogisticRegression/<数据集名>
# =============================================================================
def get_input_file_and_output_dir(
    ...
    while True:
        ...
        if os.path.exists(file_path):
            ...
            output_dir = os.path.join(base_output_dir, "LogisticRegression", subdir)
...
def train_model(X_train, y_train):
    ...
    常用超参数说明：
      - C              : 正则化强度的倒数，越小正则化越强
      - penalty        : 正则化类型（'l2' 岭回归 / 'l1' Lasso）
      - solver         : 优化算法（'lbfgs' 适合小数据集，'saga' 支持 l1）
      - max_iter       : 最大迭代次数
      - class_weight   : 处理类别不平衡（'balanced' 自动按比例加权）
    注意：逻辑回归对特征尺度敏感，需要先标准化。
    """
...
def evaluate_model(model, scaler, X_test, y_test, output_dir: str):
    ...
    ax_roc.plot(fpr, tpr, color='steelblue', lw=2,
                label=f'Logistic Regression ROC (AUC = {auc:.4f})')
    ...
    ax_roc.set_title('ROC 曲线 - 信用风险评估模型（逻辑回归）', fontsize=15, fontweight='bold')
...
def plot_feature_importance(model, feature_names, output_dir: str):
    """
    逻辑回归没有"特征重要性"，使用系数绝对值来衡量各特征的贡献。
    系数越大（绝对值），该特征对预测结果的影响越大。
    正系数（红色）→ 增大坏客户概率；负系数（蓝色）→ 降低坏客户概率。
    """
    coef = model.coef_[0]
    ...
    colors = ['#d73027' if coef[i] > 0 else '#4575b4' for i in indices]
    ax_fi.barh(
        [feature_names[i] for i in reversed(indices)],
        [importance[i] for i in reversed(indices)],
        color=[colors[j] for j in reversed(range(len(indices)))]
    )
    ax_fi.set_xlabel('系数绝对值（特征影响力）', fontsize=12)
    ax_fi.set_title('逻辑回归特征重要性（系数绝对值）', fontsize=14, fontweight='bold')
...
def plot_shap_summary(model, scaler, X_train, output_dir: str):
    ...
    shap.summary_plot(shap_values, X_train, plot_type='dot', show=False, max_display=15)
...
def main():
    ...
    print("     ├── feature_importance.png 特征重要性（系数绝对值）")
    if SHAP_AVAILABLE:
