...
def get_input_file_and_output_dir(
    ...
    while True:
        ...
        if os.path.exists(file_path):
            ...
            output_dir = os.path.join(base_output_dir, "LightGBM", subdir)
...
def train_model(X_train, y_train):
    ...
    常用超参数说明：
      ...
      - max_depth         : 树的最大深度（-1 表示不限制）
      - learning_rate     : 学习率，越小越保守
      - num_leaves        : 每棵树最大叶子数，控制复杂度（LightGBM 特有）
      - subsample         : 行采样比例，防止过拟合
      - colsample_bytree  : 列采样比例，防止过拟合
      - min_child_samples : 叶子节点最少样本数，防止过拟合
...
def evaluate_model(model, X_test, y_test, output_dir: str):
    ...
    ax_roc.plot(fpr, tpr, color='steelblue', lw=2,
                label=f'LightGBM ROC (AUC = {auc:.4f})')
    ...
    ax_roc.set_title('ROC 曲线 - 信用风险评估模型（LightGBM）', fontsize=15, fontweight='bold')
...
def plot_feature_importance(model, output_dir: str):
    """
    使用 LightGBM 内置的 plot_importance 绘制特征重要性图。
    importance_type 可选：'split'（分裂次数）/ 'gain'（平均增益）
    """
...
def main():
    print("=" * 60)
    print("  银行客户信用风险评估模型 - 基于 LightGBM")
    print("=" * 60 + "\n")
